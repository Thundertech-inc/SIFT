...
[TAtt("Sift|Sift")] // 2020/04/06 05:27:57 PM
public class gsSift : GS
{
  ...
  // G# Enums >>>>>
  // <<<<< G# cFields
  [Header("gsSift Settings")] public uint2 imageSize;[HideInInspector] public uint2 _imageSize;
  ...
  // G# ExecuteInEditor >>>>>
  // <<<<< G# G_property
  public GSift G { get { if (gSift == null) AddComputeBuffer(ref gSift, 1); gSift.GetData(); return gSift[0]; } set { gSift[0] = value; gSift.SetData(); } }
  // G# G_property >>>>>
  // <<<<< G# kernelWrappers
  public void Step1_ExtremaDetection() { var g = G; Gpu(gSift_Step1_ExtremaDetection, g.nImageSize, new { gSift }, new { OriginalImage }, new { BlurredImages }, new { Guassian }, new { FeaturePntIDs }); }
  // G# kernelWrappers >>>>>
  // <<<<< G# AssignEnums
  // G# AssignEnums >>>>>
  // <<<<< G# AssignConsts
  ...
  protected override bool onRenderObject()
  {
    ...
    // <<<<< G# RenderSetValues
    GpuSetValues(material, new { gSift }, new { image }, new { OriginalImage }, new { BlurredImages }, new { FeaturePntIDs }, new { _PaletteTex });
    // G# RenderSetValues >>>>>
    var g = G;
    uint N = 0;
  ...
  public RWStructuredBuffer<GSift> gSift;
...
#if (!gs_shader)
  // <<<<< G# kernels
  int kernel_gSift_Step1_ExtremaDetection;[numthreads(numthreads2, numthreads2, 1)]
  void gSift_Step1_ExtremaDetection
#if (gs_compute)
    ...
    if (all(id.xy < g.nImageSize))
    {
      ...
      else if (g.processCtrl == 1)
      {
        ...
        for (int m = 0; m < g.guassMatrixSize.y; m++)
        {
          ...
          {
            uint2 nm = new uint2((uint)n, (uint)m);
            //uint guaID = id_to_i(new uint2((uint)n, (uint)m), g.guassMatrixSize);
            uint guaID = id_to_i(nm, g.guassMatrixSize);
            //int2 imageID = new int2(n + id.x - ((g.guassMatrixSize.x - 1) / 2), m + id.y - ((g.guassMatrixSize.y - 1) / 2));
            int2 imageID = (int2)(nm + id.xy - (g.guassMatrixSize - 1) / 2);
            ...
            if (any(imageID < 0) || any(imageID >= (int2)g.nImageSize)) color.xyz += Guassian[guaID] * 0.5f;
            else { uint imID = id_to_i((uint2)imageID, g.nImageSize); float4 colorE = GetBeforeBlurPixel(g.oTracking, imID); color.xyz += Guassian[guaID] * colorE.xyz; }
          }
        }
          ...
          for (int k = 0; k < 3; k++)
          {
            for (int q = 0; q < 3; q++)//y
            {
              for (int p = 0; p < 3; p++)//x
              {
                ...
                float4 c = comparing - current;
                c.w = 1;
                if (all(c.xyz > 0)) counter1++; else if (all(c < 0)) counter2++;
...
#endif
  {
    ...
    if (i < n)
    {
      ...
      uint2 currentSize = g.imageSize / (uint)(1 << (int)currentOct);
  ...
  float4 frag_DrawSphere(v2f i) { float2 uv = i.uv; float r = dot(uv, uv); float4 color = i.color; if (r > 1.0f || color.w == 0) return f0000; float3 n = new float3(uv, r - 1), _LightDir = new float3(0.321f, 0.766f, -0.557f); float lightAmp = max(0.0f, dot(n, _LightDir)); float4 diffuseLight = (lightAmp + UNITY_LIGHTMODEL_AMBIENT) * color; float spec = max(0, (lightAmp - 0.95f) / 0.05f); color = lerp(diffuseLight, f1111, spec / 4); color.w = 1; return color; }
  float4 frag_DrawLine(v2f i) { float3 p0 = i.p0, p1 = i.p1; float lineRadius = i.ti.w; float2 uv = i.uv; float r = dot(uv, uv), r2 = lineRadius * lineRadius; float4 color = i.color; float3 p10 = p1 - p0; float lp10 = length(p10); if (uv.x < 0) r /= r2; else if (uv.x > lp10) { uv.x -= lp10; r = dot(uv, uv) / r2; } else { uv.x = 0; r = dot(uv, uv) / r2; } if (r > 1.0f || color.w == 0) return f0000; float3 n = new float3(uv, r - 1), _LightDir = new float3(0.321f, 0.766f, -0.557f); float lightAmp = max(0.0f, dot(n, _LightDir)); float4 diffuseLight = (lightAmp + UNITY_LIGHTMODEL_AMBIENT) * color; float spec = max(0, (lightAmp - 0.95f) / 0.05f); color = lerp(diffuseLight, f1111, spec / 4); color.w = 1; return color; }
  float4 frag_DrawArrow(v2f i) { float3 p0 = i.p0, p1 = i.p1; float lineRadius = i.ti.w; float2 uv = i.uv; float r = dot(uv, uv), r2 = lineRadius * lineRadius; float4 color = i.color; float3 p10 = p1 - p0; float lp10 = length(p10); if (uv.x < 0) r /= r2; else if (uv.x > lp10 - lineRadius * 3 && abs(uv.y) > lineRadius) { uv.x -= lp10; uv = rotate_sc(uv, -sign(uv.y) * 0.5f, 0.866025404f); uv.x = 0; r = dot(uv, uv) / r2; } else if (uv.x > lp10) { uv.x -= lp10; r = dot(uv, uv) / r2; } else { uv.x = 0; r = dot(uv, uv) / r2; } if (r > 1.0f || color.w == 0) return f0000; float3 n = new float3(uv, r - 1), _LightDir = new float3(0.321f, 0.766f, -0.557f); float lightAmp = max(0.0f, dot(n, _LightDir)); float4 diffuseLight = (lightAmp + UNITY_LIGHTMODEL_AMBIENT) * color; float spec = max(0, (lightAmp - 0.95f) / 0.05f); color = lerp(diffuseLight, f1111, spec / 4); color.w = 1; return color; }
  float4 frag_DrawLineSegment(v2f i) { float3 p0 = i.p0, p1 = i.p1; float lineRadius = i.ti.w; float2 uv = i.uv; float r = dot(uv, uv), r2 = lineRadius * lineRadius; float4 color = i.color; float3 p10 = p1 - p0; float lp10 = length(p10); uv.x = 0; r = dot(uv, uv) / r2; if (r > 1.0f || color.w == 0) return f0000; float3 n = new float3(uv, r - 1), _LightDir = new float3(0.321f, 0.766f, -0.557f); float lightAmp = max(0.0f, dot(n, _LightDir)); float4 diffuseLight = (lightAmp + UNITY_LIGHTMODEL_AMBIENT) * color; float spec = max(0, (lightAmp - 0.95f) / 0.05f); color = lerp(diffuseLight, f1111, spec / 4); color.w = 1; return color; }
  ...
  float4 frag_DrawQuad(RWStructuredBuffer<Color> v, v2f i, uint2 imageSize) { GSift g = G; return (float4)(i.color * c_f4(v[id_to_i((uint2)(i.uv * (float2)imageSize), imageSize)])); }
  ...
  float4 frag_DrawSIFT(uint octave, uint layer, v2f i, uint2 imageSize) { GSift g = G; return i.color * c32_f4(GetColorFromBuffer(octave, layer, id_to_i((uint2)(i.uv * (float2)imageSize), imageSize))); }
...
#endif
  {
    ...
    switch (roundi(i.ti.z))
    {
      ...
      case drawTexture2D: uint m = roundu(i.tSIFTo.x), n = roundu(i.tSIFTl.x); color = frag_DrawSIFT(m, n, i, g.imageSize / (uint)(1 << (int)m)); break;
